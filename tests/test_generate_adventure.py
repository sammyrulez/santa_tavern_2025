...
class AdventureTestCase(unittest.TestCase):
    def test_generate_adventure_direct(self):
        ...
        assert any(
            f.suffix == ".json" for f in files
        ), "File JSON non trovato nella cartella di output"
        assert any(
            f.suffix == ".md" for f in files
        ), "File Markdown non trovato nella cartella di output"
        ...
        with open(md_file, "r", encoding="utf-8") as f:
            ...
            assert (
                verdict.result
            ), f"L'avventura non è stata giudicata valida: {verdict.feedback}"
